...
if __name__ == '__main__':
    ...
    if not os.path.isdir(ckpt_path):
        raise FileNotFoundError(f'Checkpoint directory not found: {ckpt_path}. Please place pretrained checkpoints under ckpt/<dataset>/<model>.')
    if args.dataset == 'cifar10':
        ...
        if len(ckpts) < args.num_model:
            raise ValueError(f'Need at least {args.num_model} checkpoints in {ckpt_path}, but found {len(ckpts)}.')
        ...
        if not os.path.isdir(imagenet100_path):
            raise FileNotFoundError(f'ImageNet100 directory not found: {imagenet100_path}. Pass --imagenet100_path to point to the dataset.')
        classes = sorted(os.listdir(imagenet100_path), key=lambda x: int(x))
        class_to_idx = {cls: i for i, cls in enumerate(classes)}
        imagenet_train = datasets.ImageFolder(root=imagenet100_path, transform=transforms.ToTensor())
        imagenet_train.class_to_idx = class_to_idx
        imagenet_train.samples = [(p, class_to_idx[os.path.basename(os.path.dirname(p))]) for p, _ in imagenet_train.samples]
