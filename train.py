...
if not os.path.isfile(training_data_path):
    raise FileNotFoundError(f'UE file not found: {training_data_path}. Pass --ue with a file name under UEs/cifar10 without the .pkl suffix.')
...
if args.arch == 'resnet18':
    ...
    net = VisionTransformer(image_size=32, patch_size=4, num_layers=6, num_heads=3, hidden_dim=192, mlp_dim=768, dropout=0.1, attention_dropout=0.1, num_classes=num_classes)
elif args.arch == 'vit-small/4':
    net = VisionTransformer(image_size=32, patch_size=4, num_layers=8, num_heads=6, hidden_dim=384, mlp_dim=1536, dropout=0.1, attention_dropout=0.1, num_classes=num_classes)
elif args.arch == 'vit-base/4':
    net = VisionTransformer(image_size=32, patch_size=4, num_layers=12, num_heads=12, hidden_dim=768, mlp_dim=3072, dropout=0.1, attention_dropout=0.1, num_classes=num_classes)
net = net.to(device)
...
for epoch in range(args.epochs):
    ...
    for i, (inputs, labels) in enumerate(train_loader, 0):
        ...
        if args.defense == 'at' or args.defense == 'colorat':
            outputs, loss = MadrysLoss(epsilon=args.std / 255, distance='L_inf')(net, inputs, labels, optimizer)
        else:
            outputs = net(inputs)
        ...
        loss.backward()
        optimizer.step()
    print('[Epoch：%d/%d] loss: %.3f Train Acc: %.3f' % (epoch + 1, args.epochs, running_loss / len(train_loader), 100.0 * correct / total))
    running_loss = 0.0
    if (epoch + 1) % 5 == 0:
        net.eval()
        correct = 0
        ...
        for i, (inputs, labels) in enumerate(test_loader, 0):
            ...
            total += labels.size(0)
            correct += predicted.eq(labels).sum().item()
        print('Test Acc: %.2f' % (100.0 * correct / total))
